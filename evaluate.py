...
@torch.no_grad()
def validate_echo(model, iters=6):
    """ Peform validation using the Sintel (train) split """
    model.eval()
    results = {}

    val_dataset = datasets.EchoSynthetic(split = 'test')
    epe_list = []
    ...
    results['synthetic'] = np.mean(epe_list)
    return results
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        elif args.dataset == 'echo':
            validate_echo(model.module, iters=args.iters)
