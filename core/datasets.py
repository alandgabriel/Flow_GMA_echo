...
class MpiSintel(FlowDataset):
    def __init__(self, aug_params=None, split='training', root='/home/aromero/SINTEL', dstype='clean',
...
class EchoSynthetic (FlowDataset):
    def __init__(self, aug_params=None, split = 'training', root= '/home/alan/Documents/finalProject/syntheticDatabase/data'):
        super(EchoSynthetic, self).__init__(aug_params)
        root = osp.join(root,split)
        for fab in os.listdir(root):
            for vist in os.listdir(osp.join(root, fab)):
                for pat in os.listdir(osp.join(root, fab, vist)):
                    augF = [d for d in os.listdir(osp.join(root,fab,vist,pat)) if 'aug' in d]
                    for augf in augF:
                        imgL = sorted(glob(osp.join(root,fab,vist,pat,augf, '*.png')))
                        for i in range (len(imgL) - 1):
                            self.image_list+= [[imgL[i], imgL[i+1]]]
                            self.extra_info+= [(fab + '-' + vist + '-' + pat, i)]
                        self.flow_list += sorted(glob(osp.join(root,fab,vist,pat,'flow', '*.flo')))
...
def fetch_dataloader(args, TRAIN_DS='C+T+K+S+H'):
    ...
    if args.stage == 'chairs':
        ...
        if TRAIN_DS == 'C+T+K+S+H':
            ...
            train_dataset = 100*sintel_clean + 100*sintel_final + 200*kitti + 5*hd1k

        elif TRAIN_DS == 'C+T+K/S':
            train_dataset = 100*sintel_clean + 100*sintel_final

    elif args.stage == 'echo':
        aug_params = {'crop_size': args.image_size, 'min_scale': -0.2, 'max_scale': 0.6, 'do_flip': True}
        train_dataset = EchoSynthetic(aug_params)
