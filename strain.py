...
def farneback(imgF):
    ...
    return flowSeq/10, np.mean(eTime)
    ...
    for fab in os.listdir(root):
        for pac in os.listdir(os.path.join(root,fab,'A4C')):
            ...
            for pthA in aug_pth:
                ...
                for ixS in range(len(echofiles)):
                    msk = cv2.resize(plt.imread(maskfiles[ixS]),(960,960),interpolation = cv2.INTER_AREA)
                    epe_gma.append(np.mean(np.sqrt(np.sum((flowSeq[:,:,:,ixS][msk != 0]- read_flo_file(echofiles[ixS])[msk != 0])**2, axis=1))))
                    epe_far.append(np.mean(np.sqrt(np.sum((flowSeq_far[:,:,:,ixS] [msk !=0]- read_flo_file(echofiles[ixS])[msk != 0])**2, axis=1))))
                
                pred_curves_far,gls_far = pred_strain(pth,flowSeq_far,segments)
                
                GLSgdt.append(gls_gdt)
                GLSF.append(gls_far)
                GLSG.append(gls_gma)
                
                for ii in range(len(gdt_curves)):

                    predC = pred_curves[ii,:]
                    predCF = pred_curves_far [ii,:]
                    gdtC = gdt_curves[ii,:len(predC)]
                    slope, bias = np.polyfit(gdtC, predC,1)
                    slopeG.append(slope)
                    biasG.append(bias)
                    LOAG.append(1.96*np.std((gdtC*slope + bias)- predC))
                    slope, bias = np.polyfit(gdtC, predCF,1)
                    slopeF.append(slope)
                    biasF.append(bias)
                    LOAF.append(1.96*np.std((gdtC*slope + bias)- predC))

                    #mean_errorG.append( np.mean(predC-gdtC))
                    #var_errorG.append(np.std(predC-gdtC)*1.96)
                    #mean_errorF.append( np.mean(predCF-gdtC))
                    #var_errorF.append(np.std(predCF-gdtC)*1.96)
                    _,_,flagNG = pg.multivariate_normality(np.stack((gdtC,predC),axis=1))
                    if flagNG:
                        corr,pval = pearsonr(gdtC, predC)
                        corr_valsG.append(corr)
                        pvalG.append(pval)
                    else:
                        corr,pval = spearmanr(gdtC, predC)
                        corr_valsG.append(corr)
                        pvalG.append(pval)

                    _,_,flagNF = pg.multivariate_normality(np.stack((gdtC,predCF),axis=1))
                    if flagNF:
                        corr,pval = pearsonr(gdtC, predCF)
                        corr_valsF.append(corr)
                        pvalF.append(pval)
                    else:
                        corr,pval = spearmanr(gdtC, predCF)
                        corr_valsF.append(corr)
                        pvalF.append(pval)

                    #mi_valsG.append(mi(list(gdtC.reshape(-1,1)), list(predC.reshape(-1,1))))
                    #mi_valsF.append(mi(list(gdtC.reshape(-1,1)), list(predCF.reshape(-1,1))))
                
                #dict1 = {'EPE_GMA':epe_gma,'EPE_Far':epe_far}
                dict1 = { 'EPE_GMA':epe_gma,'EPE_Far':epe_far,'LOA_GMA':LOAG,'LOA_Far':LOAF, 'slope_GMA':slopeG,'slope_Far':slopeF,'bias_GMA':biasG,'bias_Far':biasF,'corr_GMA': corr_valsG,'corr_Far': corr_valsF,'pval_GMA': pvalG,'pval_Far': pvalF, 'GLS_gdt':GLSgdt, 'GLS_GMA':GLSG, 'GLS_Far':GLSF}   
                df = pd.DataFrame(dict([ (k,pd.Series(v)) for k,v in dict1.items() ]))
                # saving the dataframe
                df.to_csv('eval_syn_aug2.csv')
                #df.to_csv('eval_test')
                #df.to_csv('eval_syn.csv')# training dataset,without augmentation and GMA model
                #df.to_csv('eval_syn_aug.csv')# training dataset,without augmentation and Augmented-GMA model
